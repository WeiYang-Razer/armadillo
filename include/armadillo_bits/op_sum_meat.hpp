...
inline
void
op_sum_omit::apply(Cube<typename T1::elem_type>& out, const OpCube<T1,op_sum_omit>& in)
  {
  arma_debug_sigprint();
...
inline
void
op_sum_omit::apply_proxy_noalias(Cube<typename T1::elem_type>& out, const ProxyCube<T1>& P, const uword dim, functor is_omitted)
  {
  arma_debug_sigprint();
  ...
  if(dim == 0)
    ...
    for(uword slice=0; slice < P_n_slices; ++slice)
      ...
      for(uword col=0; col < P_n_cols; ++col)
        ...
        for(i=0, j=1; j < P_n_rows; i+=2, j+=2)
          {
          const eT tmp1 = P.at(i,col,slice);
          const eT tmp2 = P.at(j,col,slice);
