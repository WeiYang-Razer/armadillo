...
enable_if2
  ...
  typename T1::pod_type
  >::result
var(const T1& X, const uword norm_type, const elem_opts::omit_indicator<omit_mode>& indicator)
  {
  arma_debug_sigprint();
  
  return op_var_omit::var_vec(X, norm_type, indicator);
  }
...
enable_if2
  <
  is_arma_type<T1>::value,
  const mtOp<typename T1::pod_type, T1, op_var_omit>
  >::result
var(const T1& X, const uword norm_type, const uword dim, const elem_opts::omit_indicator<omit_mode>&)
  {
  arma_debug_sigprint();
  ...
  return mtOp<typename T1::pod_type, T1, op_var_omit>(X, norm_type, omit_mode_and_dim);
  }
