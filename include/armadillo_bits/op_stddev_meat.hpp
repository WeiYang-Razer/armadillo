...
inline
void
op_stddev_omit::apply(Mat<typename T1::pod_type>& out, const mtOp<typename T1::pod_type, T1, op_stddev_omit>& in)
  {
  arma_debug_sigprint();
  ...
  const uword norm_type         = in.aux_uword_a;
  const uword omit_mode_and_dim = in.aux_uword_b;
  
  uword omit_mode = 0;
  uword dim       = 0;
  
  if( (omit_mode_and_dim == 10) || (omit_mode_and_dim == 20) )  { dim = 0; }
  if( (omit_mode_and_dim == 11) || (omit_mode_and_dim == 21) )  { dim = 1; }
  ...
  if(U.is_alias(out))
    ...
    if(omit_mode == 1)  { op_stddev_omit::apply_noalias(tmp, U.M, norm_type, dim, is_omitted_1); }
    if(omit_mode == 2)  { op_stddev_omit::apply_noalias(tmp, U.M, norm_type, dim, is_omitted_2); }
    
    out.steal_mem(tmp);
    }
  else
    {
    if(omit_mode == 1)  { op_stddev_omit::apply_noalias(out, U.M, norm_type, dim, is_omitted_1); }
    if(omit_mode == 2)  { op_stddev_omit::apply_noalias(out, U.M, norm_type, dim, is_omitted_2); }
    }
  }
...
inline
void
op_stddev_omit::apply_noalias(Mat<typename get_pod_type<in_eT>::result>& out, const Mat<in_eT>& X, const uword norm_type, const uword dim, functor is_omitted)
  {
  arma_debug_sigprint();
  ...
  if(dim == 0)
    {
    arma_debug_print("op_stddev_omit::apply_noalias(): dim = 0");
    ...
    if(X_n_rows > 0)
      ...
      for(uword col=0; col < X_n_cols; ++col)
        {
        out_mem[col] = std::sqrt( op_var_omit::direct_var(X.colptr(col), X_n_rows, norm_type, is_omitted, work) );
        }
      }
    }
  else
  if(dim == 1)
    {
    arma_debug_print("op_stddev_omit::apply_noalias(): dim = 1");
    ...
    if(X_n_cols > 0)
      ...
      for(uword row=0; row < X_n_rows; ++row)
        ...
        out_mem[row] = std::sqrt( op_var_omit::direct_var(tmp.memptr(), tmp.n_elem, norm_type, is_omitted, work) );
        }
      }
    }
  }
...
inline
typename T1::pod_type
op_stddev_omit::stddev_vec(const T1& X, const uword norm_type, const elem_opts::omit_indicator<omit_mode>&)
  {
  arma_debug_sigprint();
  ...
  return std::sqrt( op_var_omit::direct_var(U.M.memptr(), U.M.n_elem, norm_type, is_omitted, work) );
  }
