...
inline
void
op_sp_sum_omit::apply(Mat<eT>& out, const mtSpReduceOp<eT, T1, op_sp_sum_omit>& in)
  {
  arma_debug_sigprint();
  ...
  if(omit_mode == 1)  { op_sp_sum_omit::apply(out, p, dim, is_omitted_1); }
  if(omit_mode == 2)  { op_sp_sum_omit::apply(out, p, dim, is_omitted_2); }
  }
...
inline
void
op_sp_sum_omit::apply(Mat<eT>& out, const SpProxy<T1>& p, const uword dim, functor is_omitted)
  {
  arma_debug_sigprint();
  ...
  if(dim == 0) // find the sum in each column
    ...
    for(uword i=0; i < N; ++i)
      ...
      out_mem[it.col()] += is_omitted(tmp) ? eT_zero : tmp;
  ...
  if(dim == 1)  // find the sum in each row
    ...
    for(uword i=0; i < N; ++i)
      ...
      out_mem[it.row()] += is_omitted(tmp) ? eT_zero : tmp;
